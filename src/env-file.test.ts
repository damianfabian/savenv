...
describe('env-file.parseEnv', () => {
  ...
  it('parses encrypted entries', () => {
    const lines = parseEnv("API_KEY=hidevars('AbCdEf==')");
...
describe('env-file mutation helpers', () => {
  ...
  it('setEncryptedEntry replaces a plain entry with encrypted form', () => {
    ...
    expect(serializeEnv(after)).toBe("A=hidevars('CIPHER')\n");
  });
...
describe('env-file file IO', () => {
  ...
  let envPath: string;
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'hidevars-env-'));
